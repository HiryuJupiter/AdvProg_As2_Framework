...
public class FlyweightGlobalSpeed : MonoBehaviour
{
    ...
    //Const
    const float StartingSpeed = 1f;
    const float StartingMaxSpeed = 1f;
    ...
    private void Update()
    {
        ...
        if (!SpeedIncreasing)
        {
            Speed = Mathf.Lerp(Speed, StartingSpeed, Time.deltaTime * 2f);
        }
    }
    #endregion
